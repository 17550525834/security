...
//	private static final String TAG = "ItemDialog";  
	...
	private List<zuo.biao.library.model.Dictionary.ResBean> dics;
	...
	public ItemDialog(Context context, List<Dictionary.ResBean> items, String title,
					  int requestCode, OnDialogItemClickListener listener) {
		super(context, R.style.MyDialog);
		isObject = true;
		this.context = context;
	...
	private ArrayAdapter<Dictionary.ResBean> obadapter;
	@Override
	protected void onCreate(Bundle savedInstanceState) {  
		...
		if(isObject)
			obadapter = new ArrayAdapter<Dictionary.ResBean>(context,R.layout.item_dialog_item,dics);
		else
			adapter = new ArrayAdapter<String>(context, R.layout.item_dialog_item, items);
		if(isObject)
			lvItemDialog.setAdapter(obadapter);
		else
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		if (listener != null) {
			if(isObject)
				listener.onDialogItemClick(requestCode, position, obadapter == null ? null : obadapter.getItem(position).getDictionaryName());
			else
